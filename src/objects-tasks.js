...
function mergeObjects(objects) {
  ...
  return Object.values(objects).reduce((acc, cur) => {
...
function sellTickets(queue) {
  ...
  return queue.reduce((cansell, bill) => {
    ...
    if (bill === 25) {
      ...
      if (!ticket25) return false;
      ticket25 -= 1;
      ticket50 += 1;
    } else if (bill === 100) {
      if (ticket25 && ticket50) {
        ticket25 -= 1;
        ticket50 -= 1;
      } else if (ticket25 >= 3) {
        ticket25 -= 3;
...
function group(array, keySelector, valueSelector) {
  const res = array.reduce(function map(acc, val) {
